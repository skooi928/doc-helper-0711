...
import { TaskTreeProvider, SortMode } from '../providers/taskTreeProvider';
...
export function registerTaskCommands(
    ...
    const addTaskCommand = vscode.commands.registerCommand('doc-helper-0711.addTask', async () => {
        ...
        const priority = prioritySelection?.value || 'medium';
        ...
        if (selectedFile === undefined) return; // User cancelled
        ...
        if (selectedDeadline === undefined) return; // User cancelled
    ...
    const toggleTaskCommand = vscode.commands.registerCommand('doc-helper-0711.toggleTask', async (taskId: string) => {
    ...
    // Edit task command
    const editTaskCommand = vscode.commands.registerCommand('doc-helper-0711.editTask', async (taskTreeItem: any) => {
        ...
        if (!selectedPriority) return; // User cancelled
    ...
    // Delete task command
    const deleteTaskCommand = vscode.commands.registerCommand('doc-helper-0711.deleteTask', async (taskTreeItem: any) => {
        const taskId = taskTreeItem.task.id;
    ...
    // Jump to linked file command
    const jumpToFileCommand = vscode.commands.registerCommand('doc-helper-0711.jumpToTaskFile', async (taskTreeItem: any) => {
    ...
    // Sort commands
    const sortByPriorityCommand = vscode.commands.registerCommand('doc-helper-0711.sortTasksByPriority', () => {
        taskTreeProvider.setSortMode(SortMode.Priority);
        vscode.window.showInformationMessage('Tasks sorted by priority');
    });

    const sortByCreationCommand = vscode.commands.registerCommand('doc-helper-0711.sortTasksByCreation', () => {
        taskTreeProvider.setSortMode(SortMode.CreationOrder);
        vscode.window.showInformationMessage('Tasks sorted by creation order');
    });

    const sortAlphabeticallyCommand = vscode.commands.registerCommand('doc-helper-0711.sortTasksAlphabetically', () => {
        taskTreeProvider.setSortMode(SortMode.Alphabetical);
        vscode.window.showInformationMessage('Tasks sorted alphabetically');
    });

    const sortByStatusCommand = vscode.commands.registerCommand('doc-helper-0711.sortTasksByStatus', () => {
        taskTreeProvider.setSortMode(SortMode.Status);
        vscode.window.showInformationMessage('Tasks sorted by status');
    });

    const sortByDeadlineCommand = vscode.commands.registerCommand('doc-helper-0711.sortTasksByDeadline', () => {
        taskTreeProvider.setSortMode(SortMode.Deadline);
        vscode.window.showInformationMessage('Tasks sorted by deadline');
    });

    const toggleSortCommand = vscode.commands.registerCommand('doc-helper-0711.toggleTaskSort', async () => {
    ...
    context.subscriptions.push(
        ...
        deleteTaskCommand,
        jumpToFileCommand,
        sortByPriorityCommand,
        sortByCreationCommand,
        sortAlphabeticallyCommand,
        sortByStatusCommand,
        sortByDeadlineCommand,
        toggleSortCommand
    );
}